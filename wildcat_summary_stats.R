...
# m1 is a 012 genotype matrix
m1 = read.table(args[1])
m1 = m1[,-1]

# Set summary stats for aborted simulations to NA
if (dim(m1)[1] < 95) {
...
write.table(t(as.matrix(ssrep)), paste("sstable.txt",sep=""), sep="\t",col.names=F,row.names=F, append=T)